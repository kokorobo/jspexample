...
<c:choose>
	<c:when test="${param.start == null }">
		<c:set var= "listDto" value="${dao.list(1) }"/>
	</c:when>
	<c:otherwise>
...
<table border="1">
	...
	<c:choose>
		...
		<c:otherwise> <c:set var="start" value="${param.start }"/>
		</c:otherwise>
	</c:choose>
	<c:choose>
		<c:when test="${start > 1 }">
			<button type="button" 
			onclick="location.href='list.jsp?start=${start-1}'">
			이전</button>
		</c:when>
		<c:otherwise>
	...
	<c:forEach begin="1" end="${(totalPage/3)+1 }" step="1" var="cnt">
